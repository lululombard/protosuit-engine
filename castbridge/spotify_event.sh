#!/bin/bash
# Spotify event handler for spotifyd
# Called via onevent config - publishes raw events to MQTT for CastBridge
...
JSON=$(printf '{"event":"%s","track_id":"%s","position_ms":%s,"duration_ms":%s,"name":"%s","artists":"%s","album":"%s","covers":"%s","volume":%s}' \
    "${PLAYER_EVENT:-}" \
    "${TRACK_ID:-}" \
    "${POSITION_MS:-0}" \
    "${DURATION_MS:-0}" \
