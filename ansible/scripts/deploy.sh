...
if ansible-playbook -i inventory/hosts.yml playbooks/$PLAYBOOK -v; then
    ...
    echo "3. Check service status: sudo systemctl status protosuit-engine"
    echo "4. View logs: sudo journalctl -u protosuit-engine -f"
    echo ""
    echo -e "${BLUE}Useful Commands:${NC}"
    echo "  Start service: sudo systemctl start protosuit-engine"
    echo "  Stop service: sudo systemctl stop protosuit-engine"
    echo "  Restart service: sudo systemctl restart protosuit-engine"
    echo "  View logs: sudo journalctl -u protosuit-engine -f"
else
    echo ""
