...
// RGB Ring Shader - Glowing Ring with Noise
...
uniform float innerRadius;    // Inner hole radius (default: 0.6)
uniform float noiseScale;     // Noise scale (default: 0.65)
uniform float glowIntensity;  // Glow intensity (default: 1.0)
uniform vec3 color1;          // Primary color (default: purple)
uniform vec3 color2;          // Secondary color (default: cyan)
uniform vec3 color3;          // Background ring color (default: dark blue)
...
void draw( out vec4 _FragColor, in vec2 vUv )
{
    ...
    v0 = light1(1.0, 10.0, d0) * glowIntensity;
    ...
    d = distance(uv, pos);
    v1 = light2(1.5, 5.0, d) * glowIntensity;
...
void mainImage( out vec4 fragColor_out, in vec2 fragCoord )
{
    ...
    fragColor_out.rgb = mix(backgroundColor, col.rgb, col.a);
    fragColor_out.a = 1.0;
}
