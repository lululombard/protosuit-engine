...
uniform float speed;          // Zoom speed (default: 1.0)
uniform float cameraAngle;    // Camera rotation angle (default: 0.0)
uniform float cameraHeight;   // Camera height (default: 1.5)
uniform vec3 colorX;          // X-axis color (default: red)
uniform vec3 colorY;          // Y-axis color (default: green)
uniform vec3 colorZ;          // Z-axis color (default: blue)
uniform vec3 backgroundColor; // Background color (default: gray)
...
void mainImage( out vec4 fragColor_out, in vec2 fragCoord ) {
   float time = (100.0 + iTime * 100.0 * speed) / 100.0;
   ...
   float beta = 3.14159 / 4.0 + 0.2 + cameraAngle;
   float s = sin(beta);
   float c = cos(beta);

   // Initialize the Camera
   Camera C = camera(
       vec3(2.0*c, 2.0*s, cameraHeight),
       vec3(0.0, 0.0, 0.0),
       50.0
   );
   ...
   fragColor_out = vec4(backgroundColor, 1.0);
   ...
   if(intersect_sphere(R,S,t,max_t)) {
      ...
      if(coord <= 2){ // hit a solid cube
          // Apply axis colors
          if(coord == 0) col = colorX;
          else if(coord == 1) col = colorY;
          else col = colorZ;
